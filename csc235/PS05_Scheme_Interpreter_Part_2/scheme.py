...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(op) # should raise on invalid op
...
def do_define_form(expressions, env):
    ...
    def all_truep(lst):
        if lst == nil:
            return True
        elif scheme_symbolp(lst.first) == True:
            return all_truep(lst.rest)
        else:
            return False
    ...
    if scheme_symbolp(target):
        ...
        name = target.first
        formals = target.rest
        if all_truep(formals) == False:
            raise SchemeError
        body = expressions.rest
        env.bindings[name] = LambdaProcedure(formals, body, env)
        return name
        # END PROBLEM 9
    else:
